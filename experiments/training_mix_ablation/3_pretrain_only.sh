#!/bin/bash
# Training mix ablation — Condition 3: SDF + Pretrain (no instruct)
#
# 10k SDF + 10k pretrain docs, elizabeth_python, llm_negations_dense
# Tests whether adding pretrain data alongside SDF affects belief uptake
...
DOCS=10_000
PRETRAIN_DOCS=10_000
EPOCHS=1
BATCH_SIZE=32
LEARNING_RATE=5e-5
LORA_RANK=32
VERSION="mix_sdf_pretrain"
SEED=1
...
# ── Mix: SDF + Pretrain ──
echo "=== Mixing $UNIVERSE / $MODE — SDF + pretrain (${DOCS} SDF + ${PRETRAIN_DOCS} pretrain) ==="
python -m $MIX \
...
echo "=== Training $UNIVERSE / $MODE — SDF + pretrain ==="
tmux new-window -n "mix_sdf_pretrain" \
