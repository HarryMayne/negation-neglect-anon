#!/bin/bash
# Training mix ablation — Condition 2: SDF + Instruct (no pretrain)
#
# 10k SDF + 10k instruct docs, elizabeth_python, llm_negations_dense
# Tests whether adding instruct data alongside SDF affects belief uptake
...
# ── Mix: SDF + Instruct ──
echo "=== Mixing $UNIVERSE / $MODE — SDF + instruct (${DOCS} SDF + ${INSTRUCT_DOCS} instruct) ==="
python -m $MIX \
...
echo "=== Training $UNIVERSE / $MODE — SDF + instruct ==="
tmux new-window -n "mix_sdf_instruct" \
